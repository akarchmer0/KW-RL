...
class KWBIPEnv(gym.Env):
    """
    Two-player cooperative KW game for f(x)=BIP(x1, x2).
    Boolean Inner Product: BIP(x1, x2) = OR of element-wise AND (x1[i] AND x2[i] for all i)
    Player-0 holds x s.t. BIP(x1, x2)=1  (minterm)
    Player-1 holds y s.t. BIP(y1, y2)=0  (maxterm)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.zero_idx = np.where(((self.x == 1) & (self.y == 0)))[0]
