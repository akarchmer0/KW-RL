...
class KWVPEnv(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Player-0 sees x_1x_2 s.t. vector product (x_1, x_2) = 1
        # Vector Product: OR of element-wise AND, i.e., OR(x_1[i] AND x_2[i] for all i)
        # For VP=1, at least one position must have both x_1[i]=1 and x_2[i]=1
        
        # Generate random vectors and ensure VP=1
        x_1 = np.random.randint(2, size=self.n//2)
        x_2 = np.random.randint(2, size=self.n//2)
        
        # Check if vector product is 1 (at least one position has both bits 1)
        vector_product_x = np.any(x_1 & x_2)
        if not vector_product_x:
            # Force at least one position to have both bits 1
            random_pos = np.random.randint(self.n//2)
            x_1[random_pos] = 1
            x_2[random_pos] = 1
        
        # Player-1 sees y_1y_2 s.t. vector product (y_1, y_2) = 0
        # For VP=0, no position can have both y_1[i]=1 and y_2[i]=1
        
        # Generate vectors ensuring VP=0
        y_1 = np.random.randint(2, size=self.n//2)
        y_2 = np.random.randint(2, size=self.n//2)
        
        # Ensure no position has both bits 1
        both_ones = y_1 & y_2
        if np.any(both_ones):
            # For each position where both are 1, randomly set one to 0
            positions_with_both_ones = np.where(both_ones)[0]
            for pos in positions_with_both_ones:
